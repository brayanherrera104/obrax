...
ROLE_DEFAULTS={
 'Gerente':{k:['view','create','edit'] for k,_ in MODULES},
 'Administrativo':{k:['view','create','edit'] for k in ['dashboard','clients','projects','budgets','quotations','payables','receivables','cashflow']},
 'Residente':{'dashboard':['view'],'projects':['view'],'control':['view','create','edit'],'apus':['view']},
 'Consulta':{'dashboard':['view'],'projects':['view'],'control':['view']}
}
...
def posted_permissions(form):
 ...
 for key,_ in MODULES:
  ...
  for action in ACTIONS:
   if form.get(f'perm_{key}_{action}')=='1':acts.append(action)

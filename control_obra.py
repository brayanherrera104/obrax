...
def project_financials(c,project_id,company_id,contract,budget,actual,progress):
 ...
 payable=sum(float(x['amount'] or 0) for x in c.execute("SELECT amount FROM project_costs WHERE project_id=? AND company_id=? AND payment_status='Pendiente'",(project_id,company_id)).fetchall());target=(budget*progress/100) if budget>0 else None;deviation=(actual-target) if target is not None else None;projection_reliable=bool(progress>0 and target and actual >= target*(.90 if progress>=100 else .50));projected_cost=(actual/progress*100) if projection_reliable else None;projected_profit=(contract-projected_cost) if projected_cost is not None else None;projected_margin=(projected_profit/contract*100) if projected_profit is not None and contract else None
 ...
 return {'billed':billed,'retentions':retentions,'net_expected':net_expected,'collected':collected,'receivable_pending':receivable_pending,'payable':payable,'projected_cost':projected_cost,'projected_profit':projected_profit,'projected_margin':projected_margin,'projection_reliable':projection_reliable,'alert_level':level,'alert_title':title,'alert_message':message}

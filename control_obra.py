...
def project_control(project_id):
 ...
 costs=c.execute('SELECT * FROM project_costs WHERE project_id=? AND company_id=? ORDER BY cost_date DESC,id DESC',(project_id,cid)).fetchall();actual=sum(x['amount'] or 0 for x in costs);budget=project_budget_total(c,project_id,cid);contract=p['value'] or 0;expected_profit=contract-budget;real_profit=contract-actual;deviation=actual-budget if budget else 0;c.close()
 return render_template('project_control.html',project=p,costs=costs,budget=budget,actual=actual,expected_profit=expected_profit,real_profit=real_profit,deviation=deviation,today=date.today().isoformat())

...
def receivables():
    ...
    totals=c.execute('SELECT * FROM project_receivables WHERE company_id=?',(cid,)).fetchall();billed=sum(float(x['amount'] or 0) for x in totals);retained=sum(calc(x)[0] for x in totals);net_total=sum(calc(x)[1] for x in totals);collected=sum(float(x['paid_amount'] or 0) for x in totals);pending=sum(calc(x)[2] for x in totals);overdue=sum(calc(x)[2] for x in totals if calc(x)[2]>0 and x['due_date'] and x['due_date']<today);projects=c.execute('SELECT id,name FROM projects WHERE company_id=? ORDER BY name',(cid,)).fetchall();c.close()
    return render_template('receivables.html',rows=rows,projects=projects,billed=billed,retained=retained,net_total=net_total,collected=collected,pending=pending,overdue=overdue,today=today,status_filter=status,project_filter=project_filter)
